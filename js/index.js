...
addEventListener("DOMContentLoaded", function(){
    ...
    if(location.pathname == "/") {
        ...
        // get years in fandom
        var FandomJoined = getYearDifference(new Date(2021, 1, 2), new Date())
        ...
        // IDK HOW TO DO THIS IN CSS SORRY OK?
        if(navigator.userAgent.indexOf("Mobile") > 0) {
            var AvatarDiv = document.querySelector("div > div > div > div.col.s3");
            var TextContentDiv = document.querySelector("div > div > div > div.col.s8");
            
            TextContentDiv.classList.add("mobile");
            AvatarDiv.classList.add("mobile");
            AvatarDiv.classList.remove("s3");
            AvatarDiv.classList.add("s6");
        };
    };

    if(location.pathname.includes("kokscraft-online")) {
        var Minecraft = {
            reconnect: () => Minecraft.Chat = new WebSocket("ws://localhost:" + WSPORT),
            sendMessage: text => Minecraft.Chat.send("message", text),
            Chat: new WebSocket("ws://localhost:" + WSPORT)
        };

        document.querySelector("iframe[chat]").contentDocument.body.innerHTML += `<style>*{font-family: consolas;font-size:small};</style>`

        var onError = () => {
            document.querySelector("div[noconnection]").style.dispay = "unset";
            document.querySelector("div[connection]").style.dispay = "none";
        };

        var onSuccess = () => {
            document.querySelector("div[no-connection]").style.dispay = "none";
            document.querySelector("div[connection]").style.dispay = "unset";
        };

        function renderMessage(text) {
            var Message = text.replaceColorCodes();
            Message.appendChild(document.createElement("br"));
            document.querySelector("iframe[chat]").contentDocument.body.appendChild(Message);
        };

        renderMessage("§aThis Is Chat Tunnel Between Your Browser And Minecraft!")

        // Minecraft.Chat.addEventListener("error", onError);
        // Minecraft.Chat.addEventListener("open", onSuccess);
    };
 ...
 * @param {Date} newdate 
 */
function getYearDifference(olddate, newdate) {
